...
using Symbolics: Symbolic, symstruct, juliatype, symbolic_getproperty, symbolic_setproperty!, symbolic_constructor
...
@test isequal(Symbolics.arguments(xa), Symbolic[Symbolics.unwrap(x), Val{:a}()])
xa = Symbolics.unwrap(symbolic_setproperty!(x, :a, 10))
@test Symbolics.operation(xa) == setfield!
@test isequal(Symbolics.arguments(xa), Symbolic[Symbolics.unwrap(x), Meta.quot(:a), 10])
@test Symbolics.symtype(xa) == Int

xb = Symbolics.unwrap(symbolic_setproperty!(x, :b, 10))
@test Symbolics.operation(xb) == setfield!
@test isequal(Symbolics.arguments(xb), Symbolic[Symbolics.unwrap(x), Meta.quot(:b), 10])
